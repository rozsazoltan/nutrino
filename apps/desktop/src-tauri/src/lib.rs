...
use axum::{
    extract::{
        ...
        ConnectInfo, DefaultBodyLimit, Path as AxumPath, Query, State as AxumState,
    },
...
const MOBILE_HANDOFF_PENDING_TTL_MS: i64 = 15 * 60 * 1000;
    ...
    tauri::Builder::default()
        ...
        .setup(|app| {
            ...
            if let Some(window) = app.get_webview_window("main") {
                if let Ok(settings) = read_desktop_settings(&db_path) {
                    ...
                    let launched_hidden =
                        std::env::args().any(|arg| arg == "--hidden" || arg == "--tray");
                    if launched_hidden
                        && settings.run_in_background
                        && settings.start_hidden_to_tray
                    {
                        let _ = window.hide();
                    }
                }
            ...
            let start_server_item =
                MenuItemBuilder::with_id("start_server", "Start server").build(app)?;
            let stop_server_item =
                MenuItemBuilder::with_id("stop_server", "Stop server").build(app)?;
        ...
        .on_window_event(|window, event| {
            if let WindowEvent::CloseRequested { api, .. } = event {
                ...
                if read_bool_setting(&state.db_path, "remember_window_state", false)
                    .unwrap_or(false)
                {
                ...
                let run_in_background =
                    read_bool_setting(&state.db_path, "run_in_background", false).unwrap_or(false);
                let close_to_tray =
                    read_bool_setting(&state.db_path, "close_to_tray", false).unwrap_or(false);
...
fn safe_download_file_name(raw: &str) -> String {
    let safe = raw
        ...
        .to_string();
    if safe.is_empty() {
        format!("nutrino-export-{}.bin", Utc::now().format("%Y%m%d-%H%M%S"))
    } else {
        safe
    }
}

fn configured_default_download_dir(
    app: &tauri::AppHandle,
    db_path: &Path,
...
fn unique_download_path(dir: &Path, filename: &str) -> PathBuf {
    let safe = safe_download_file_name(filename);
...
fn save_bytes_to_default_download_dir_blocking(
    app: &tauri::AppHandle,
    db_path: &Path,
    filename: &str,
    bytes: &[u8],
    ...
    if bytes.is_empty() {
        return Err("Refusing to save an empty mobile export file.".to_string());
    }
    ...
    let path = unique_download_path(&dir, filename);
    {
        ...
        file.sync_all()
            .map_err(|error| format!("Could not sync export file to disk: {error}"))?;
    }
    let metadata = std::fs::metadata(&path)
        .map_err(|error| format!("Could not verify saved export file: {error}"))?;
    if metadata.len() == 0 {
        let _ = std::fs::remove_file(&path);
        return Err("Saved mobile export file is empty.".to_string());
    }
    if metadata.len() != bytes.len() as u64 {
        let _ = std::fs::remove_file(&path);
        return Err(format!(
            "Saved mobile export size mismatch: {} / {} bytes.",
            metadata.len(),
            bytes.len()
        ));
    }
...
fn decode_mobile_export_base64(base64_text: &str) -> std::result::Result<Vec<u8>, String> {
    let normalized = base64_text.trim();
    if normalized.is_empty() {
        return Err("Missing mobile export data.".to_string());
    }
    general_purpose::STANDARD
        .decode(normalized)
        .or_else(|_| general_purpose::URL_SAFE.decode(normalized))
        .or_else(|_| general_purpose::URL_SAFE_NO_PAD.decode(normalized))
        .map_err(|error| format!("Could not decode mobile export data: {error}"))
}

fn save_base64_file_to_default_download_dir_blocking(
    app: &tauri::AppHandle,
    db_path: &Path,
    filename: &str,
    base64_text: &str,
...
fn append_mobile_handoff_result_chunk_blocking(
    ...
    let request =
        db_get_mobile_handoff_request(&conn, request_id).map_err(|error| error.to_string())?;
    ...
    if is_last_chunk {
        if let Some(expected_size) = payload.total_size {
            if part_size != expected_size {
                let _ = std::fs::remove_file(&part_path);
                return Err(format!(
                    "Mobile export size mismatch: {part_size} / {expected_size} bytes."
                ));
            }
        }
        if part_size == 0 {
        ...
        if let Some(expected_size) = payload.total_size {
            if metadata.len() != expected_size {
                let _ = std::fs::remove_file(&final_path);
                return Err(format!(
                    "Finalized mobile export size mismatch: {} / {expected_size} bytes.",
                    metadata.len()
                ));
            }
        }
...
#[tauri::command]
fn get_default_download_dir(
    app: tauri::AppHandle,
    state: State<'_, AppState>,
) -> std::result::Result<String, String> {
    configured_default_download_dir(&app, &state.db_path)
        .map(|path| path.to_string_lossy().to_string())
}

#[tauri::command]
async fn save_file_to_default_download_dir(
    app: tauri::AppHandle,
    state: State<'_, AppState>,
    filename: String,
    bytes: Vec<u8>,
...
#[tauri::command]
async fn download_and_open_update_installer(
    app: tauri::AppHandle,
    url: String,
    asset_name: Option<String>,
) -> std::result::Result<String, String> {
    tauri::async_runtime::spawn_blocking(move || {
        download_and_open_update_installer_blocking(&app, &url, asset_name)
    })
    .await
    .map_err(|error| format!("Update installer task failed: {error}"))?
}

fn download_and_open_update_installer_blocking(
    app: &tauri::AppHandle,
    url: &str,
    asset_name: Option<String>,
) -> std::result::Result<String, String> {
...
fn open_downloaded_update_installer(path: &Path) -> std::result::Result<(), String> {
    #[cfg(target_os = "windows")]
    {
        ...
        if extension == "msi" {
            ...
            return Err(format!(
                "Unsupported Windows update installer type: .{extension}"
            ));
        }
        return Ok(());
    }
...
fn optional_nutrients_from_json(raw: Option<String>) -> HashMap<String, f64> {
    clean_optional_nutrients(
        raw.and_then(|value| serde_json::from_str::<HashMap<String, f64>>(&value).ok()),
    )
}
...
fn parse_optional_nutrients_csv(
...
fn parse_name_i18n_csv(
    ...
    for language in SUPPORTED_I18N_LANGUAGES {
        ...
        if let Some(value) =
            get_csv(headers, record, &column).filter(|value| !value.trim().is_empty())
        {
...
fn init_database(path: &Path) -> Result<()> {
    ...
    let _ = conn.execute(
        "ALTER TABLE mobile_handoff_requests ADD COLUMN result_saved_path TEXT",
        [],
    );

    let _ = conn.execute(
        "ALTER TABLE weight_logs ADD COLUMN bmi REAL NOT NULL DEFAULT 0",
        [],
    );
    let _ = conn.execute(
        "ALTER TABLE foods ADD COLUMN catalog_kind TEXT NOT NULL DEFAULT 'food'",
        [],
    );
    let _ = conn.execute("ALTER TABLE foods ADD COLUMN note TEXT", []);
    let _ = conn.execute("ALTER TABLE foods ADD COLUMN barcode TEXT", []);
    let _ = conn.execute(
        "ALTER TABLE foods ADD COLUMN name_i18n TEXT NOT NULL DEFAULT '{}'",
        [],
    );
    let _ = conn.execute(
        "ALTER TABLE ingredients ADD COLUMN name_i18n TEXT NOT NULL DEFAULT '{}'",
        [],
    );
    let _ = conn.execute(
        "ALTER TABLE foods ADD COLUMN optional_nutrients_json TEXT NOT NULL DEFAULT '{}'",
        [],
    );
    let _ = conn.execute(
        "ALTER TABLE ingredients ADD COLUMN optional_nutrients_json TEXT NOT NULL DEFAULT '{}'",
        [],
    );
    migrate_catalog_kind_ingredients(&conn)?;
    let _ = conn.execute("ALTER TABLE recipes ADD COLUMN note TEXT", []);
    let _ = conn.execute(
        "ALTER TABLE recipes ADD COLUMN name_i18n TEXT NOT NULL DEFAULT '{}'",
        [],
    );
    let _ = conn.execute(
        "ALTER TABLE recipes ADD COLUMN extra_kcal REAL NOT NULL DEFAULT 0",
        [],
    );
    let _ = conn.execute(
        "ALTER TABLE intakes ADD COLUMN item_type TEXT NOT NULL DEFAULT 'food'",
        [],
    );
    ...
    let _ = conn.execute(
        "ALTER TABLE activities ADD COLUMN name_i18n TEXT NOT NULL DEFAULT '{}'",
        [],
    );
...
fn migrate_server_password(conn: &Connection) -> Result<()> {
    let existing_password: Option<String> = conn
        .query_row(
            "SELECT value FROM settings WHERE key = 'server_password'",
            [],
            |row| row.get(0),
        )
        .optional()?;
    if existing_password.is_none() {
        let old_token: Option<String> = conn
            .query_row(
                "SELECT value FROM settings WHERE key = 'server_token'",
                [],
                |row| row.get(0),
            )
            .optional()?;
...
fn read_desktop_settings(path: &Path) -> Result<DesktopSettings> {
    ...
    Ok(DesktopSettings {
        ...
        default_download_dir: setting(&conn, "default_download_dir")
            .ok()
            .map(|value| value.trim().to_string())
            .filter(|value| !value.is_empty()),
...
fn write_desktop_settings(path: &Path, settings: &DesktopSettings) -> Result<()> {
    ...
    set_setting(
        ...
        &settings.remember_window_state.to_string(),
    )?;
    set_setting(
        &conn,
        "launch_at_startup",
    ...
    set_setting(
        &conn,
        "run_in_background",
        &settings.run_in_background.to_string(),
    )?;
    set_setting(
        &conn,
        "auto_start_server",
        &settings.auto_start_server.to_string(),
    )?;
    set_setting(&conn, "close_to_tray", &settings.close_to_tray.to_string())?;
    set_setting(
        &conn,
        "start_hidden_to_tray",
        &settings.start_hidden_to_tray.to_string(),
    )?;
    set_setting(
        &conn,
        "check_prerelease_updates",
        &settings.check_prerelease_updates.to_string(),
    )?;
    set_setting(
        &conn,
        "default_download_dir",
        settings.default_download_dir.as_deref().unwrap_or(""),
    )?;
    if let Some(value) = settings.window_x {
        set_setting(&conn, "window_x", &value.to_string())?;
    }
    if let Some(value) = settings.window_y {
        set_setting(&conn, "window_y", &value.to_string())?;
    }
    if let Some(value) = settings.window_width {
        set_setting(&conn, "window_width", &value.to_string())?;
    }
    if let Some(value) = settings.window_height {
        set_setting(&conn, "window_height", &value.to_string())?;
    }
...
fn set_launch_at_startup(enabled: bool, start_hidden_to_tray: bool) -> Result<()> {
    ...
    if enabled {
        ...
        let command = if start_hidden_to_tray {
            format!("{quoted} --hidden")
        } else {
            quoted
        };
...
#[cfg(not(windows))]
fn set_launch_at_startup(_enabled: bool, _start_hidden_to_tray: bool) -> Result<()> {
    Ok(())
}
...
async fn start_api_server(port: u16, state: State<'_, AppState>) -> Result<ServerStatus, String> {
    start_api_server_internal(port, &state)
        .await
...
async fn start_api_server_internal(port: u16, state: &AppState) -> Result<ServerStatus> {
    ...
    let router = Router::new()
        ...
        .route(
            ...
            post(mobile_handoff_response)
                ...
                .layer(RequestBodyLimitLayer::new(
                    MOBILE_HANDOFF_RESPONSE_BODY_LIMIT,
                )),
        )
        ...
        .layer(DefaultBodyLimit::max(MOBILE_HANDOFF_RESPONSE_BODY_LIMIT))
        .layer(
            CorsLayer::new()
                .allow_origin(Any)
                .allow_headers(Any)
    ...
    tauri::async_runtime::spawn(async move {
        ...
        if let Err(err) = result {
            eprintln!("nutrino API server stopped with error: {err}");
        }
    });
    ...
    for ingredient in ingredients {
        writer
            .write_record([
                ...
                optional_nutrient_csv_value(
                    &ingredient.optional_nutrients,
                    "saturated_fat_per_100g",
                ),
...
fn import_ingredients_csv(
    ...
    for (index, result) in reader.records().enumerate() {
        ...
        if name.trim().is_empty() {
            row_errors.push("name is required".into());
        }
        let ingredient = Ingredient {
            ...
            serving_size_g: parse_optional_number(
                &headers,
                &record,
                "serving_size_g",
                &mut row_errors,
            ),
            kcal_per_100g: parse_required_number(
                &headers,
                &record,
                "kcal_per_100g",
                &mut row_errors,
            ),
            carbs_per_100g: parse_required_number(
                &headers,
                &record,
                "carbs_per_100g",
                &mut row_errors,
            ...
            protein_per_100g: parse_required_number(
                &headers,
                &record,
                "protein_per_100g",
                &mut row_errors,
            ),
            sugars_per_100g: parse_number_default(
                &headers,
                &record,
                "sugars_per_100g",
                0.0,
            ...
            fiber_per_100g: parse_number_default(
                &headers,
                &record,
                "fiber_per_100g",
                0.0,
            ...
            salt_per_100g: parse_number_default(
                &headers,
                &record,
                "salt_per_100g",
                0.0,
        ...
        if !row_errors.is_empty() {
            errors.push(format!("Row {row_number}: {}", row_errors.join(", ")));
            skipped += 1;
            continue;
        }
        if skip_duplicates.unwrap_or(false)
            && find_existing_ingredient_duplicate(&conn, &ingredient)
                .map_err(stringify_error)?
                .is_some()
        {
            skipped += 1;
        ...
        if let Err(err) = upsert_ingredient(&conn, &ingredient) {
            errors.push(format!("Row {row_number}: {err}"));
            skipped += 1;
        } else {
...
fn import_foods_commit(
    ...
    for row in preview.rows {
        match (row.food, row.errors.is_empty()) {
            (Some(food), true) => {
                if skip_duplicates.unwrap_or(false)
                    && find_existing_food_duplicate(&conn, &food)
                        .map_err(stringify_error)?
                        .is_some()
                {
                    skipped += 1;
...
fn save_recipe(state: State<'_, AppState>, input: RecipeInput) -> Result<RecipeDetail, String> {
    ...
    for item in input.items {
        tx.execute(
            ...
            "#,
            params![
                format!("recipe-item-{}", Uuid::new_v4()),
                recipe_id,
                item.food_id,
                item.amount_g,
                now
...
fn save_activity(
    state: State<'_, AppState>,
    input: ActivityInput,
) -> Result<ActivityDefinition, String> {
    let conn = open_conn(&state.db_path).map_err(stringify_error)?;
...
#[tauri::command]
fn accept_sync_inbox(
    state: State<'_, AppState>,
    entry_id: String,
) -> Result<SyncInboxCommitResult, String> {
    let conn = open_conn(&state.db_path).map_err(stringify_error)?;
    let payload_json: String = conn
        ...
        .map_err(stringify_error)?;
    let mut payload: SyncPushRequest =
        serde_json::from_str(&payload_json).map_err(stringify_error)?;
...
#[tauri::command]
fn update_sync_inbox_payload(
    state: State<'_, AppState>,
    entry_id: String,
    payload_json: String,
) -> Result<SyncInboxEntry, String> {
    let conn = open_conn(&state.db_path).map_err(stringify_error)?;
    let mut payload: SyncPushRequest =
        serde_json::from_str(&payload_json).map_err(stringify_error)?;
    strip_private_mobile_diary(&mut payload);
    let normalized_payload_json =
        serde_json::to_string_pretty(&payload).map_err(stringify_error)?;
...
fn merge_catalog_item(
    ...
    let conn = open_conn(&state.db_path).map_err(stringify_error)?;
    merge_catalog_item_internal(&conn, &kind, &alias_id, &canonical_id, None)
        .map_err(stringify_error)
}

#[tauri::command]
fn list_catalog_duplicate_suggestions(
    state: State<'_, AppState>,
) -> Result<Vec<CatalogDuplicateSuggestion>, String> {
    let conn = open_conn(&state.db_path).map_err(stringify_error)?;
    ...
    for activity in activities {
        writer
            .write_record([
                ...
                activity.code,
                activity.name,
                name_i18n_to_json(&activity.name_i18n),
                activity.description.unwrap_or_default(),
                activity.activity_type,
                activity.met.to_string(),
                activity.kcal_per_min.to_string(),
            ])
            .map_err(stringify_error)?;
    }
    let bytes = writer.into_inner().map_err(stringify_error)?;
...
fn import_activities_csv(
    ...
    for (index, result) in reader.records().enumerate() {
        ...
        let kcal_per_min =
            parse_number_default(&headers, &record, "kcal_per_min", 0.0, &mut row_errors);
        if !row_errors.is_empty() {
            errors.push(format!("Row {row_number}: {}", row_errors.join(", ")));
            skipped += 1;
            continue;
        ...
        let activity = ActivityDefinition {
            ...
            code: get_csv(&headers, &record, "code")
                .filter(|v| !v.trim().is_empty())
                .unwrap_or_else(|| "custom".into()),
            name,
            name_i18n: parse_name_i18n_csv(&headers, &record),
            description: get_csv(&headers, &record, "description").filter(|v| !v.trim().is_empty()),
            activity_type: get_csv(&headers, &record, "activity_type")
                .filter(|v| !v.trim().is_empty())
                .unwrap_or_else(|| "custom".into()),
            met,
            kcal_per_min,
        ...
        if skip_duplicates.unwrap_or(false)
            && find_existing_activity_duplicate(&conn, &activity)
                .map_err(stringify_error)?
                .is_some()
        {
            skipped += 1;
        ...
        if let Err(err) = upsert_activity(&conn, &activity) {
            errors.push(format!("Row {row_number}: {err}"));
            skipped += 1;
        } else {
            inserted_or_updated += 1;
...
fn export_recipes_csv(state: State<'_, AppState>) -> Result<String, String> {
    ...
    for detail in recipes {
        let ingredients: Vec<RecipeInputItem> = detail
            .items
            .iter()
            .map(|item| RecipeInputItem {
...
fn import_recipes_csv(
    ...
    for (index, result) in reader.records().enumerate() {
        ...
        let legacy_total_weight_g =
            parse_optional_number(&headers, &record, "total_weight_g", &mut row_errors);
        let _ = legacy_total_weight_g;
        let servings_count =
            parse_optional_number(&headers, &record, "servings_count", &mut row_errors);
        let recipe_id = get_csv(&headers, &record, "recipe_id")
            .filter(|v| !v.trim().is_empty())
            .unwrap_or_else(|| format!("recipe-import-{}", name.to_lowercase().replace(' ', "-")));
        let mut items: Vec<RecipeInputItem> = vec![];

        if let Some(ingredients_json) =
            get_csv(&headers, &record, "ingredients_json").filter(|v| !v.trim().is_empty())
        {
            match serde_json::from_str::<Vec<RecipeInputItem>>(&ingredients_json) {
                Ok(parsed) => items.extend(
                    parsed
                        .into_iter()
                        .filter(|item| !item.food_id.trim().is_empty() && item.amount_g > 0.0),
                ),
            ...
            if food_id.trim().is_empty() {
                row_errors.push("food_id is required".into());
            }
            let amount_g = parse_required_number(&headers, &record, "amount_g", &mut row_errors);
            if !food_id.trim().is_empty() && amount_g > 0.0 {
                items.push(RecipeInputItem { food_id, amount_g });
            }
        }

        if items.is_empty() {
            row_errors.push("at least one ingredient is required".into());
        }
        if !row_errors.is_empty() {
            errors.push(format!("Row {row_number}: {}", row_errors.join(", ")));
            skipped += 1;
    ...
    for (_, group) in groups {
        ...
        if skip_duplicates.unwrap_or(false) {
            let incoming = Recipe {
                ...
                name_i18n: group.name_i18n.clone(),
                description: group.description.clone(),
                note: group.note.clone(),
                total_weight_g: None,
                extra_kcal: Some(extra_kcal),
                servings_count: group.servings_count,
                updated_at: now,
                deleted_at: None,
            };
            if find_existing_recipe_duplicate(&conn, &incoming)
                .map_err(stringify_error)?
                .is_some()
            {
                skipped += 1;
...
fn strip_private_mobile_diary(payload: &mut SyncPushRequest) {
    ...
    payload.skipped_items.retain(|item| {
        item.kind != "intake" && item.kind != "weight_log" && item.kind != "activity_log"
    });
}
...
fn find_existing_food_duplicate(conn: &Connection, incoming: &Food) -> Result<Option<Food>> {
    ...
    Ok(existing
        .into_iter()
        .find(|item| item.id != incoming.id && food_signature(item) == food_signature(incoming)))
}

fn find_existing_ingredient_duplicate(
    conn: &Connection,
    incoming: &Ingredient,
) -> Result<Option<Ingredient>> {
    let existing = query_active_ingredients(conn)?;
    Ok(existing.into_iter().find(|item| {
        item.id != incoming.id && ingredient_signature(item) == ingredient_signature(incoming)
    }))
}
...
fn find_existing_recipe_duplicate(conn: &Connection, incoming: &Recipe) -> Result<Option<Recipe>> {
    ...
    Ok(existing.into_iter().find(|item| {
        item.id != incoming.id && recipe_signature(item) == recipe_signature(incoming)
    }))
}

fn find_existing_activity_duplicate(
    conn: &Connection,
    incoming: &ActivityDefinition,
) -> Result<Option<ActivityDefinition>> {
    let existing = query_active_activities(conn)?;
    Ok(existing.into_iter().find(|item| {
        item.id != incoming.id && activity_signature(item) == activity_signature(incoming)
    }))
}
        ...
        for food in foods {
            if let Some(existing) = find_existing_food_duplicate(conn, food)? {
                candidates.push(MergeCandidate {
                    kind: "food".into(),
                    incoming_id: food.id.clone(),
                    incoming_name: food.name.clone(),
        ...
        for ingredient in ingredients {
            if let Some(existing) = find_existing_ingredient_duplicate(conn, ingredient)? {
                candidates.push(MergeCandidate {
                    ...
                    incoming_id: ingredient.id.clone(),
                    incoming_name: ingredient.name.clone(),
                    canonical_id: existing.id,
                    canonical_name: existing.name,
                });
            }
    ...
    if let Some(recipes) = &payload.recipes {
        for recipe in recipes {
            if let Some(existing) = find_existing_recipe_duplicate(conn, recipe)? {
                candidates.push(MergeCandidate {
                    kind: "recipe".into(),
                    incoming_id: recipe.id.clone(),
                    incoming_name: recipe.name.clone(),
                    canonical_id: existing.id,
                    canonical_name: existing.name,
                });
            }
    ...
    if let Some(activities) = &payload.activities {
        for activity in activities {
            if let Some(existing) = find_existing_activity_duplicate(conn, activity)? {
                candidates.push(MergeCandidate {
                    kind: "activity".into(),
                    incoming_id: activity.id.clone(),
                    incoming_name: activity.name.clone(),
                    canonical_id: existing.id,
                    canonical_name: existing.name,
                });
            }
...
fn find_catalog_duplicate_suggestions(
    conn: &Connection,
) -> Result<Vec<CatalogDuplicateSuggestion>> {
    ...
    for ingredient in ingredients {
        ...
        ingredient_exact
            .entry(ingredient_signature(&ingredient))
            .or_default()
            .push(item.clone());
    ...
    for activity in activities {
        ...
        activity_exact
            .entry(activity_signature(&activity))
            .or_default()
            .push(item.clone());
...
fn merge_catalog_item_internal(
    conn: &Connection,
    kind: &str,
    alias_id: &str,
    canonical_id: &str,
    ...
    match kind {
        "food" => {
            ...
            conn.execute(
                "UPDATE foods SET updated_at = ?1 WHERE id = ?2",
        ...
        "ingredient" => {
            ...
            conn.execute(
                "UPDATE ingredients SET updated_at = ?1 WHERE id = ?2",
                params![now, canonical_id],
            )?;
        }
        "recipe" => {
...
fn canonical_activity_ref(conn: &Connection, id: Option<String>) -> Result<Option<String>> {
    match id {
        Some(value) if !value.trim().is_empty() => {
            Ok(Some(resolve_alias(conn, "activity", &value)?))
        }
        _ => Ok(None),
    }
}

fn commit_sync_payload(
    conn: &Connection,
    payload: &SyncPushRequest,
) -> Result<SyncInboxCommitResult> {
    let now = now_ms();
    ...
    if let Some(foods) = &payload.foods {
        for food in foods {
            if let Some(existing) = find_existing_food_duplicate(conn, food)? {
                ...
                next.updated_at = if next.updated_at > 0 {
                    next.updated_at
                } else {
                    now
                };
    ...
    if let Some(recipe_items) = &payload.recipe_items {
        for item in recipe_items {
            ...
            let food_id = if item.food_id.starts_with("recipe:") {
                ...
                format!(
                    "ingredient:{}",
                    canonical_ingredient_ref(conn, &item.food_id)?
                )
            } else {
    ...
    for intake in &payload.intakes {
        ...
        let food_id = match item_type.as_str() {
            ...
            "ingredient" => format!(
                "ingredient:{}",
                canonical_ingredient_ref(conn, &intake.food_id)?
            ),
...
fn list_connected_devices(state: State<'_, AppState>) -> Result<Vec<ConnectedDevice>, String> {
    ...
    let mut devices = state
        .connected_devices
        .lock()
        .map_err(|_| "connected device registry lock poisoned".to_string())?;
...
fn mobile_handoff_request_from_row(
    row: &rusqlite::Row<'_>,
) -> rusqlite::Result<MobileHandoffRequest> {
    ...
    Ok(MobileHandoffRequest {
        ...
        result_saved_path: row.get(11)?,
        message: row.get(12)?,
    })
}

fn expire_mobile_handoff_requests(conn: &Connection) -> Result<usize> {
    let cutoff = now_ms() - MOBILE_HANDOFF_PENDING_TTL_MS;
    Ok(conn.execute(
        r#"UPDATE mobile_handoff_requests
           SET status = 'expired',
               responded_at = ?1,
               message = COALESCE(NULLIF(message, ''), 'The mobile request expired before the phone responded.')
...
fn db_list_mobile_handoff_requests(path: &Path) -> Result<Vec<MobileHandoffRequest>> {
    let conn = open_conn(path)?;
    expire_mobile_handoff_requests(&conn)?;
    let mut stmt = conn.prepare(
        r#"SELECT id, device_id, device_name, kind, status, created_at, responded_at, payload_json,
                  result_filename, result_mime_type, result_base64, result_saved_path, message
...
fn device_name_for_handoff(state: &AppState, device_id: &str) -> Option<String> {
    state.connected_devices.lock().ok().and_then(|devices| {
        devices
            .get(device_id)
...
fn emit_mobile_handoff_event(
    notifier: &MobileHandoffNotifier,
    event: &str,
    device_id: String,
    request_id: Option<String>,
    request_kind: Option<String>,
...
fn mobile_handoff_kind_label(kind: &str) -> &'static str {
    match kind {
...
fn queue_mobile_handoff_request_internal(
    state: &AppState,
    device_id: String,
    kind: &str,
    mut payload: serde_json::Value,
) -> Result<MobileHandoffRequest> {
    if let Some(object) = payload.as_object_mut() {
        object
            .entry("desktop_name")
            .or_insert_with(|| serde_json::json!(format!("{} Desktop", APP_NAME)));
    }
    ...
    let conn = open_conn(&state.db_path)?;
    expire_mobile_handoff_requests(&conn)?;
    let existing_pending: i64 = conn.query_row(
        "SELECT COUNT(*) FROM mobile_handoff_requests WHERE device_id = ?1 AND kind = ?2 AND status = 'pending'",
        params![device_id, kind],
        |row| row.get(0),
    )?;
    if existing_pending > 0 {
        return Err(anyhow!(
            "A {} request is already pending for this mobile. Wait for the phone response or for the request to expire.",
            mobile_handoff_kind_label(kind),
        ));
    }
    conn.execute(
        r#"INSERT INTO mobile_handoff_requests (id, device_id, device_name, kind, status, created_at, responded_at, payload_json, result_filename, result_mime_type, result_base64, result_saved_path, message)
           VALUES (?1, ?2, ?3, ?4, 'pending', ?5, NULL, ?6, NULL, NULL, NULL, NULL, NULL)"#,
        params![id, device_id, device_name, kind, now, payload_json],
    )?;
    let request = db_get_mobile_handoff_request(&conn, &id)?;
    emit_mobile_handoff_event(
        &state.handoff_notifier,
        "request_created",
...
fn db_get_mobile_handoff_request(
    conn: &Connection,
    request_id: &str,
) -> Result<MobileHandoffRequest> {
    conn.query_row(
...
#[tauri::command]
fn list_mobile_handoff_requests(
    state: State<'_, AppState>,
) -> Result<Vec<MobileHandoffRequest>, String> {
    db_list_mobile_handoff_requests(&state.db_path).map_err(stringify_error)
}

#[tauri::command]
fn request_mobile_backup_export(
    device_id: String,
    state: State<'_, AppState>,
) -> Result<MobileHandoffRequest, String> {
    queue_mobile_handoff_request_internal(&state, device_id, "backup_export", serde_json::json!({}))
        .map_err(stringify_error)
}

#[tauri::command]
fn request_mobile_ai_export(
    device_id: String,
    state: State<'_, AppState>,
) -> Result<MobileHandoffRequest, String> {
    queue_mobile_handoff_request_internal(&state, device_id, "ai_export", serde_json::json!({}))
        .map_err(stringify_error)
}

#[tauri::command]
fn send_mobile_backup_import(
    device_id: String,
    filename: String,
    ...
    queue_mobile_handoff_request_internal(
        &state,
        device_id,
        "backup_import",
        serde_json::json!({
...
#[tauri::command]
fn clear_mobile_handoff_history(state: State<'_, AppState>) -> Result<u64, String> {
    let conn = open_conn(&state.db_path).map_err(stringify_error)?;
    expire_mobile_handoff_requests(&conn).map_err(stringify_error)?;
    conn.execute(
        "DELETE FROM mobile_handoff_requests WHERE status != 'pending'",
        [],
    )
...
fn parse_android_model_from_user_agent(user_agent: &str) -> Option<String> {
    ...
    let model = after_version
        ...
        .unwrap_or(after_version)
        .trim();
    if model.is_empty() || model.eq_ignore_ascii_case("wv") || model.eq_ignore_ascii_case("mobile")
    {
        None
    } else {
...
fn friendly_device_display_name(
    device_name: Option<&String>,
    manufacturer: Option<&String>,
    model: Option<&String>,
    user_agent: Option<&String>,
    ...
    let manufacturer = manufacturer
        .map(|value| title_case_device_part(value))
        .filter(|value| !value.is_empty());
...
fn record_connected_device(headers: &HeaderMap, peer: SocketAddr, state: &ApiState, path: &str) {
    ...
    let raw_id = header_value(headers, "x-nutrino-device-id")
        .unwrap_or_else(|| format!("{}:{}", peer.ip(), user_agent.clone().unwrap_or_default()));
    ...
    let os_version = header_value(headers, "x-nutrino-os-version")
        .or_else(|| header_value(headers, "x-nutrino-device-os-version"));
    let app_channel = header_value(headers, "x-nutrino-app-channel");
    let app_version = header_value(headers, "x-nutrino-app-version");
    let display_name = friendly_device_display_name(
        device_name.as_ref(),
        manufacturer.as_ref(),
        model.as_ref(),
        user_agent.as_ref(),
...
async fn update_check_requested(
    ...
    let _ = state
        .app_handle
        .emit("nutrino-update-check-requested", payload);
...
async fn sync_pull(
    ...
    let foods = db_list_foods_for_sync(&state.db_path, since)
        .map_err(|_| StatusCode::INTERNAL_SERVER_ERROR)?;
    let ingredients = db_list_ingredients_for_sync(&state.db_path, since)
        .map_err(|_| StatusCode::INTERNAL_SERVER_ERROR)?;
    let recipes = db_list_recipes_for_sync(&state.db_path, since)
        .map_err(|_| StatusCode::INTERNAL_SERVER_ERROR)?;
    let recipe_items = db_list_recipe_items(&state.db_path, since)
        .map_err(|_| StatusCode::INTERNAL_SERVER_ERROR)?;
    let activities = db_list_activities_for_sync(&state.db_path, since)
        .map_err(|_| StatusCode::INTERNAL_SERVER_ERROR)?;
    let aliases = db_list_aliases_for_sync(&state.db_path, since)
        .map_err(|_| StatusCode::INTERNAL_SERVER_ERROR)?;
...
async fn sync_push(
    ...
    let merge_candidates =
        find_merge_candidates(&conn, &payload).map_err(|_| StatusCode::INTERNAL_SERVER_ERROR)?;
    ...
    let merge_json =
        serde_json::to_string(&merge_candidates).map_err(|_| StatusCode::BAD_REQUEST)?;
    conn.execute(
...
async fn mobile_handoff_requests(
    ...
    let conn = open_conn(&state.db_path).map_err(|_| StatusCode::INTERNAL_SERVER_ERROR)?;
    expire_mobile_handoff_requests(&conn).map_err(|_| StatusCode::INTERNAL_SERVER_ERROR)?;
    let mut stmt = conn.prepare(
        r#"SELECT id, device_id, device_name, kind, status, created_at, responded_at, payload_json,
                  result_filename, result_mime_type, result_base64, result_saved_path, message
           FROM mobile_handoff_requests
    ...
    let rows = stmt
        .query_map(params![device_id], mobile_handoff_request_from_row)
        .map_err(|_| StatusCode::INTERNAL_SERVER_ERROR)?;
    let requests = rows
...
async fn mobile_handoff_ws(
    ...
    authorize_with_optional_query_token(
        &headers,
...
async fn mobile_handoff_ws_session(
    ...
    let _ = state
        .app_handle
        .emit("nutrino-mobile-handoff-ws-closed", device_id);
}
...
async fn mobile_handoff_result_chunk(
    ...
    authorize(&headers, &state.token, state.auth_required)?;
    record_connected_device(
        &headers,
        peer,
        &state,
        "/api/v1/mobile/requests/result-chunk",
    );
    let header_device_id =
        header_value(&headers, "x-nutrino-device-id").ok_or(StatusCode::BAD_REQUEST)?;
    let app_handle = state.app_handle.clone();
    let db_path = state.db_path.clone();
    let request_id_for_task = request_id.clone();
    let device_id_for_task = header_device_id.clone();
    let ack = tauri::async_runtime::spawn_blocking(move || {
        append_mobile_handoff_result_chunk_blocking(
            &app_handle,
            &db_path,
            &request_id_for_task,
            &device_id_for_task,
...
async fn mobile_handoff_response(
    ...
    let allowed_status = matches!(
        ...
        "completed" | "rejected" | "used" | "kept" | "deleted" | "error"
    );
    if !allowed_status {
    ...
    let conn = open_conn(&state.db_path).map_err(|_| StatusCode::INTERNAL_SERVER_ERROR)?;
    expire_mobile_handoff_requests(&conn).map_err(|_| StatusCode::INTERNAL_SERVER_ERROR)?;
    let pending_exists: i64 = conn.query_row(
        "SELECT COUNT(*) FROM mobile_handoff_requests WHERE id = ?1 AND device_id = ?2 AND status = 'pending'",
        params![request_id, header_device_id],
        |row| row.get(0),
    ).map_err(|_| StatusCode::INTERNAL_SERVER_ERROR)?;
    if pending_exists == 0 {
    ...
    let result_filename = payload.result_filename.clone();
    let result_mime_type = payload.result_mime_type.clone();
    let result_base64 = payload.result_base64.clone();
    let mut result_saved_path = payload.result_saved_path.clone();
    let mut message = payload.message.clone();

    if payload.status == "completed" && result_saved_path.is_none() {
        if let Some(base64_text) = result_base64
            .as_deref()
            .filter(|value| !value.trim().is_empty())
        {
            let fallback_filename = if payload.result_mime_type.as_deref() == Some("text/markdown")
            {
                format!(
                    "nutrino-mobile-ai-export-{}.md",
                    Utc::now().format("%Y%m%d-%H%M%S")
                )
            } else {
                format!(
                    "nutrino-mobile-backup-{}.zip",
                    Utc::now().format("%Y%m%d-%H%M%S")
                )
            };
            let filename = result_filename.clone().unwrap_or(fallback_filename);
            let base64_for_save = base64_text.to_string();
            let app_handle = state.app_handle.clone();
            let db_path = state.db_path.clone();
            let saved_path = tauri::async_runtime::spawn_blocking(move || {
                save_base64_file_to_default_download_dir_blocking(
                    &app_handle,
                    &db_path,
                    &filename,
                    &base64_for_save,
            ...
            .map_err(|_| StatusCode::INTERNAL_SERVER_ERROR)?
            .map_err(|_| StatusCode::INTERNAL_SERVER_ERROR)?;
            result_saved_path = Some(saved_path.clone());
            message = Some(
                match message
                    .as_deref()
                    .map(str::trim)
                    .filter(|value| !value.is_empty())
                {
                    Some(existing) => format!("{existing} Saved to {saved_path}"),
                    None => format!("Saved to {saved_path}"),
                },
            );
        }
    }
    ...
    let changed = conn
        .execute(
           ...
           SET status = ?1,
               ...
               result_saved_path = ?6,
               message = ?7
           WHERE id = ?8 AND device_id = ?9 AND status = 'pending'"#,
            params![
                payload.status,
                now,
                result_filename,
    ...
    emit_mobile_handoff_event(
        &state.handoff_notifier,
        "request_updated",
        header_device_id.clone(),
        Some(request_id.clone()),
        None,
    );
    let _ = state
        .app_handle
        .emit("nutrino-mobile-handoff-updated", request_id);
    Ok(Json(MobileHandoffResponseAck {
        accepted: true,
        server_time: now,
    }))
}
...
fn authorize_with_optional_query_token(
    headers: &HeaderMap,
...
fn db_get_sync_inbox_entry(conn: &Connection, entry_id: &str) -> Result<SyncInboxEntry> {
    ...
    entry.replacement_candidates =
        find_replacement_candidates(conn, &entry.payload).unwrap_or_default();
    Ok(entry)
}
...
fn db_recipe_detail_from_recipe_guarded(
    path: &Path,
    recipe: Recipe,
    ...
    if !visited.insert(recipe.id.clone()) {
        return Err(anyhow!(
            "recipe ingredient cycle detected for {}",
            recipe.name
        ));
    }
    ...
    for item in raw_items {
        ...
        if let Some(recipe_ref) = item.food_id.strip_prefix("recipe:") {
            ...
            if let Some(referenced_recipe) = referenced {
                let detail =
                    db_recipe_detail_from_recipe_guarded(path, referenced_recipe, visited)?;
        ...
        if let Some((food_name, kcal_per_100g, carbs_per_100g, fat_per_100g, protein_per_100g)) =
            food
        {
            items.push(RecipeItemDetail {
                id: item.id,
    ...
    let multiplier = if total_weight_g > 0.0 {
        100.0 / total_weight_g
    } else {
        0.0
    };
...
fn parse_food_csv(db_path: &Path, csv_text: &str) -> Result<ImportPreview> {
    ...
    for (index, result) in reader.records().enumerate() {
        ...
        let food = Food {
            ...
            brand,
            note: get_csv(&headers, &record, "note").filter(|value| !value.trim().is_empty()),
            barcode: get_csv(&headers, &record, "barcode")
                .or_else(|| get_csv(&headers, &record, "ean"))
                .or_else(|| get_csv(&headers, &record, "upc"))
                .filter(|value| !value.trim().is_empty()),
            default_unit: get_csv(&headers, &record, "default_unit").unwrap_or_else(|| "g".into()),
...
fn activity_from_input(input: ActivityInput) -> Result<ActivityDefinition> {
    let name = input.name.trim().to_string();
    if name.is_empty() {
        return Err(anyhow!("activity name is required"));
    }
    if input.met < 0.0 || !input.met.is_finite() {
        return Err(anyhow!("MET must be a non-negative number"));
    }
    if input.kcal_per_min < 0.0 || !input.kcal_per_min.is_finite() {
        return Err(anyhow!("kcal_per_min must be a non-negative number"));
    }
    Ok(ActivityDefinition {
        id: input
            .id
...
fn db_query_activities(
    path: &Path,
    since: i64,
    active_only: bool,
) -> Result<Vec<ActivityDefinition>> {
    let conn = open_conn(path)?;
    let sql = if active_only {
        r#"
