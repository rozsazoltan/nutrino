...
import { PREPARATION_METHODS as preparationMethods, estimatePreparationKcal, foodSupportsPreparation, hasPieceServing, intakePreparationKcal, preparationMethodConfig } from './lib/preparation';
...
const foodPreparationMethod = ref<FoodPreparationMethod>('none');
const foodPreparationOilMl = ref<number | null>(null);
...
function hasActiveMealSheetDraft(): boolean {
  ...
  if (addMode.value === 'food') {
    ...
    return Boolean(
      ...
      foodPreparationMethod.value !== 'none' ||
      Number(foodPreparationOilMl.value || 0) > 0 ||
...
const completeMobileLanguageTranslations: Record<string, Record<string, string>> = {
    "hu": {
      ...
      "preparationMethod": "Elkészítés",
      "preparationNone": "Nincs külön elkészítés",
      "preparationNoneHint": "Az étel rögzített tápértékét használja.",
      "preparationBoiled": "Vízben főtt / párolt",
      "preparationBoiledHint": "Nem ad hozzá extra olajkalóriát.",
      "preparationAirFryer": "Levegősütő / nagyon kevés olaj",
      "preparationAirFryerHint": "Kb. 27 kcal / 100 g extra olajbevonattal számol.",
      "preparationPanLightOil": "Serpenyőben, kevés olajjal",
      "preparationPanLightOilHint": "Kb. 45 kcal / 100 g extra zsiradékkal számol.",
      "preparationPanOil": "Serpenyőben, normál olajjal",
      "preparationPanOilHint": "Kb. 90 kcal / 100 g extra zsiradékkal számol.",
      "preparationDeepFried": "Bő olajban sült",
      "preparationDeepFriedHint": "Kb. 135 kcal / 100 g extra zsiradékkal számol.",
      "preparationCustomOil": "Egyedi olajmennyiség",
      "preparationCustomOilHint": "A beírt olajmennyiségből számol extra kcal-t.",
      "preparationOilMl": "Felhasznált olaj (ml)",
      "preparationKcalPreview": "becsült sütési zsiradék",
      "preparationExtraShort": "elkészítés"
    },
    "de": {
      "home": "Start",
      "diary": "Tagebuch",
      ...
      "preparationNone": "No extra preparation",
      "preparationNoneHint": "Use the food value as stored.",
      "preparationBoiled": "Boiled / steamed",
      "preparationBoiledHint": "No extra oil calories are added.",
      "preparationAirFryer": "Air fryer / very little oil",
      "preparationAirFryerHint": "Adds an estimated 27 kcal / 100 g for a light oil coating.",
      "preparationPanLightOil": "Pan fried, little oil",
      "preparationPanLightOilHint": "Adds an estimated 45 kcal / 100 g.",
      "preparationPanOil": "Pan fried, regular oil",
      "preparationPanOilHint": "Adds an estimated 90 kcal / 100 g.",
      "preparationDeepFried": "Deep fried",
      "preparationDeepFriedHint": "Adds an estimated 135 kcal / 100 g.",
      "preparationCustomOil": "Custom oil amount",
      "preparationCustomOilHint": "Track the exact oil amount used for this entry.",
      "preparationOilMl": "Oil used (ml)",
      "preparationKcalPreview": "estimated cooking fat",
      "preparationExtraShort": "preparation"
    },
    "fr": {
      "home": "Accueil",
      "diary": "Journal",
...
function intakeKcal(intake: Intake) {
  ...
  const base = food ? calculateKcal(food, intake.amount_g) : 0;
  return base + intakePreparationKcal(intake);
}

function catalogItemHasPieceServing(food?: Food) {
  return hasPieceServing(food);
}

function servingQtyForAmount(amountG: number, food?: Food) {
  if (!catalogItemHasPieceServing(food)) return null;
...
function intakePreparationLabel(entry: Intake) {
  if (!entry.preparation_method || entry.preparation_method === 'none') return '';
  const config = preparationMethodConfig(entry.preparation_method);
  const parts = [t(config.labelKey)];
  const kcal = intakePreparationKcal(entry);
...
const selectedCatalogAmountHint = computed(() => {
  ...
  if (!item || !amount || amount <= 0 || !catalogItemHasPieceServing(item)) return '';
  const serving = Number(item.serving_size_g || 0);
  if (!serving) return '';
...
const selectedCatalogSupportsPreparation = computed(() => foodSupportsPreparation(selectedCatalog.value));

function resetPreparationForm() {
  foodPreparationMethod.value = 'none';
  foodPreparationOilMl.value = null;
}

function preparationKcalForAmount(amountG: number) {
  return estimatePreparationKcal({
    method: foodPreparationMethod.value,
    amountG,
    oilMl: foodPreparationOilMl.value,
  });
}

const selectedPreparationPreview = computed(() => {
  if (!selectedCatalogSupportsPreparation.value) return null;
  let item = selectedCatalog.value;
  const rawAmount = Number(foodAmount.value || 0);
  if (!item || !rawAmount || rawAmount <= 0) return null;
  item = item.id.startsWith('recipe:') ? buildCustomRecipeSnapshot(item) : item;
  const serving = catalogItemHasPieceServing(item) ? Number(item.serving_size_g || 0) : 0;
  const amountG = foodUnit.value === 'serving' && serving > 0 ? rawAmount * serving : rawAmount;
  const kcal = preparationKcalForAmount(amountG);
  return { kcal, amountG: Math.round(amountG * 10) / 10 };
});

function foodPreparationPayload(amountG: number) {
  if (!selectedCatalogSupportsPreparation.value || foodPreparationMethod.value === 'none') {
    return { preparation_method: null, preparation_oil_ml: null, preparation_kcal: null };
  }
  const method = foodPreparationMethod.value;
  const config = preparationMethodConfig(method);
  const oilMl = method === 'custom_oil'
    ? Math.max(0, Math.round(Number(foodPreparationOilMl.value || 0) * 10) / 10)
    : config.defaultOilMl || null;
  return {
    preparation_method: method,
    preparation_oil_ml: oilMl,
    preparation_kcal: preparationKcalForAmount(amountG),
  };
}
...
const selectedMealAmountPreview = computed(() => {
  ...
  const preparationKcal = selectedCatalogSupportsPreparation.value ? preparationKcalForAmount(amountG) : 0;
  return {
    amountG: Math.round(amountG * 10) / 10,
    kcal: calculateKcal(item, amountG) + preparationKcal,
    baseKcal: calculateKcal(item, amountG),
    preparationKcal,
...
function chooseCatalogItem(item: Food) {
  ...
  recipeCustomizeOpen.value = false;
  const hasServing = catalogItemHasPieceServing(item);
  foodUnit.value = hasServing ? 'serving' : 'g';
  foodAmount.value = hasServing ? 1 : null;
  resetPreparationForm();
  search.value = '';
  catalogPickerOpen.value = false;
}
...
function addFoodLog() {
  ...
  item = item.id.startsWith('recipe:') ? buildCustomRecipeSnapshot(item) : item;
  const servingSize = catalogItemHasPieceServing(item) ? item.serving_size_g || 0 : 0;
  ...
  const payload = {
    ...
    meal_type: addMealType.value,
    amount_g: amountG,
    unit: foodUnit.value === 'serving' && servingSize > 0 ? 'serving' as const : 'g' as const,
    serving_qty: foodUnit.value === 'serving' && servingSize > 0 ? rawAmount : null,
    food_snapshot_json: foodSnapshot(item),
    ...foodPreparationPayload(amountG),
    pending_sync: false,
    updated_at: now,
  };
  if (editingIntakeId.value) {
...
function editIntake(entry: Intake) {
  ...
  const food = foodFromIntake(entry);
  const canUseServing = catalogItemHasPieceServing(food);
  foodUnit.value = entry.unit === 'serving' && canUseServing ? 'serving' : 'g';
  foodAmount.value = foodUnit.value === 'serving' && entry.serving_qty ? entry.serving_qty : entry.amount_g;
  foodPreparationMethod.value = entry.preparation_method || 'none';
  foodPreparationOilMl.value = entry.preparation_oil_ml ?? null;
}
      ...
      <article v-for="section in sections" :key="section.key" class="card meal-card" :data-tour="section.key === 'breakfast' ? 'meal-logging' : undefined">
        ...
        <div v-else class="entry-list">
          <div v-for="entry in entriesForSection(section)" :id="`intake-entry-${entry.id}`" :key="entry.id" class="entry-row" :class="{ 'review-highlight': highlightedReviewIntakeId === entry.id }" @pointerdown="startEntryLongPress('intake', entry.id, $event)" @pointerup="clearEntryLongPress" @pointercancel="clearEntryLongPress" @contextmenu.prevent="entryActionSheet = { kind: 'intake', id: entry.id }">
            <div><b>{{ itemTitle(foodFromIntake(entry)) }}</b><small>{{ amountLabel(entry.amount_g, foodFromIntake(entry)) }} · {{ intakeKcal(entry) }} kcal<template v-if="intakePreparationLabel(entry)"> · {{ intakePreparationLabel(entry) }}</template></small></div>
      ...
      <div v-if="addMode" class="sheet-backdrop app-overlay" @click.self="requestCloseSheet">
        ...
        <template v-else-if="addMode === 'food'">
            ...
            <div class="picker-list grouped-picker">
              <div v-if="selectedCatalog" class="picker-group selected-picker-group">
                ...
                <button class="picker-row selected" @click="chooseCatalogItem(selectedCatalog)">
                  ...
                  <strong>{{ catalogItemHasPieceServing(selectedCatalog) ? `${Math.round(selectedCatalog.serving_size_g || 0)}g/db` : 'g' }}</strong>
                </button>
              </div>
              <template v-if="catalogSearchActive">
                <div v-if="catalogExactPickerItems.length" class="picker-group">
                  ...
                  <button v-for="item in catalogExactPickerItems" :key="`picker-exact-${item.id}`" class="picker-row" :class="selectedCatalogId === item.id ? 'selected' : ''" @click="chooseCatalogItem(item)">
                    ...
                    <strong>{{ catalogItemHasPieceServing(item) ? `${Math.round(item.serving_size_g || 0)}g/db` : 'g' }}</strong>
                  </button>
                </div>
          ...
          <div v-if="foodFormVisible" class="selected-entry-fields">
            <div class="unit-toggle" :class="{ disabled: !catalogItemHasPieceServing(selectedCatalog) }">
              <button type="button" :class="foodUnit === 'g' ? 'active' : ''" @click="foodUnit = 'g'">g</button>
              <button type="button" :disabled="!catalogItemHasPieceServing(selectedCatalog)" :class="foodUnit === 'serving' ? 'active' : ''" @click="foodUnit = 'serving'">db</button>
            </div>
            ...
            <div v-if="selectedCatalogSupportsPreparation" class="preparation-panel">
              <label class="field-label">{{ t('preparationMethod') }}
                <select v-model="foodPreparationMethod" class="input">
                  <option v-for="method in preparationMethods" :key="method.key" :value="method.key">{{ t(method.labelKey) }}</option>
                </select>
              </label>
              <small class="input-help">{{ t(preparationMethodConfig(foodPreparationMethod).hintKey) }}</small>
              <label v-if="foodPreparationMethod === 'custom_oil'" class="field-label">{{ t('preparationOilMl') }}
                <input v-model.number="foodPreparationOilMl" class="input" type="number" min="0" step="0.5" inputmode="decimal" @focus="selectNumberInput" @pointerdown="clearNumberInputOnDoubleTap" />
              </label>
              <small v-if="selectedPreparationPreview?.kcal" class="preparation-kcal-preview">+{{ selectedPreparationPreview.kcal }} kcal · {{ t('preparationKcalPreview') }}</small>
            </div>
            <div v-if="selectedMealAmountPreview" class="live-nutrition-preview meal-live-preview">
              ...
              <small>{{ selectedMealAmountPreview.kcalPer100g }} kcal / 100g<span v-if="selectedMealAmountPreview.preparationKcal"> · +{{ selectedMealAmountPreview.preparationKcal }} kcal {{ t('preparationExtraShort') }}</span></small>
            </div>
