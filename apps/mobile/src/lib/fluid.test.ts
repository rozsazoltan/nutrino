...
describe('fluid tracking helpers', () => {
  it('keeps non-alcoholic fluids calorie-free', () => {
    expect(
      estimateFluidAlcoholKcal({ amountDl: 3, isAlcohol: false, kind: 'beer' }),
    ).toBe(0);
  ...
  it('estimates alcohol calories from dl-based presets', () => {
    expect(
      estimateFluidAlcoholKcal({ amountDl: 5, isAlcohol: true, kind: 'beer' }),
    ).toBe(225);
    expect(
      estimateFluidAlcoholKcal({ amountDl: 2, isAlcohol: true, kind: 'wine' }),
    ).toBe(170);
    expect(
      estimateFluidAlcoholKcal({ amountDl: 0.5, isAlcohol: true, kind: 'spirits' }),
    ).toBe(115);
  });
