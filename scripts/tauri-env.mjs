...
function repoTauriCliScript(repoRoot) {
  return path.join(repoRoot, 'scripts', 'tauri-cli.mjs');
}
...
const child = spawn(process.execPath, [repoTauriCliScript(repoRoot), ...args], {
  cwd,
  env,
  stdio: 'inherit',
  shell: false,
...
child.on('error', (error) => {
  ...
  console.error(
    'Install dependencies with `aube install`. Nutrino only uses the project-local @tauri-apps/cli; set NUTRINO_TAURI_CLI only for debugging.',
  );
  console.error(error);
  process.exit(1);
});
